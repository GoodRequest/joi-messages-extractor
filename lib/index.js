...
const init = async () => {
    const messagePromises = files.map(async (typeName) => {
        ...
        const type = await Promise.resolve(`${`joi/lib/types/${typeName}`}`).then(s => __importStar(require(s)));
    ...
    const actualData = fs_1.default.existsSync(savePath) ? (await Promise.resolve(`${savePath}`).then(s => __importStar(require(s)))).default : {};
